...
//This is the same as categories/name , also a dynamic website 
export default function Blog({params}: {params: {date: string}}){
...
function Date(props: {date:string}){
    ...
    let { data , isLoading , isError , isFetching ,  } = useQuery<prop>({
        queryFn: ()=> axios.get(process.env.NEXT_PUBLIC_BASE_FETCH_URL +'/blogsByTime', {params: {date:props.date, t:take , s:0}})
        .then((res)=>{ setCount(res.data.count); return res.data.result; }) 
        .catch(()=>{ return new Error("Server error occurred")}),

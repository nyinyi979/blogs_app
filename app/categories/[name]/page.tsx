...
function Categories(props: {name:string}){
    let [ take , setTake ] = useState(10);
    let [ grid , setGrid ] = useState(false);
    let [ count , setCount ] = useState(0);
    let { data , isLoading , isError , isFetching ,  } = useQuery<prop>({
        queryFn: ()=> axios.get(process.env.NEXT_PUBLIC_BASE_FETCH_URL +'/blogsByCategory', {params: {category:decodeURIComponent(props.name), t:take , s:0}})
        .then((res)=>{ setCount(res.data.count); return res.data.result; }) 
        .catch(()=>{ return new Error("Server error occurred")}),
        queryKey: ["blogsByCategory" , take] , 
